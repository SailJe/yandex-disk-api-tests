...
| 11 | files: копирование файла | POST copy `201`/`202` | Source и destination содержат исходные данные |
...
состояние ресурсов через GET. Обычный copy, включая E2E, допускает `201`/`202`;
при `202` helper ожидает завершения операции. Удаление пустой папки проверяется
в синхронном режиме. Cleanup принимает
